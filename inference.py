...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        "*** YOUR CODE HERE ***"
        if self.total() == 0:
            raise ValueError("Total of distribution is zero; can't sample.")
        choice = random.uniform(0, self.total())
        cumulative = 0
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles.extend(extra_positions)
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            weightedDistribution[particle] += weight
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        newPosDist = {particle: self.getPositionDistribution(gameState, particle) for particle in set(self.particles)}
        self.particles = [newPosDist[particle].sample() for particle in self.particles]
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(self.numParticles):
            combination = all_combinations[i % num_combinations]
            self.particles.append(combination)
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            weight = 1
            for i, ghostPos in enumerate(particle):
                weight *= self.getObservationProb(observation[i], gameState.getPacmanPosition(), ghostPos, self.getJailPosition(i))
            weighted_belief[particle] += weight

        weighted_belief.normalize()

        if weighted_belief.total() == 0:
            self.initializeUniformly(gameState)
        else:
            self.particles = [weighted_belief.sample() for _ in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for ghostIndex in range(self.numGhosts):
                newPosDist = self.getPositionDistribution(gameState, oldParticle, ghostIndex, self.ghostAgents[ghostIndex])
                newParticle[ghostIndex] = newPosDist.sample()
